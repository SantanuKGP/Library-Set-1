...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val newMap = w.toList.groupBy(x => x.toLower).map(x => (x._1,x._2.length))
    newMap.keys.toList.sorted.map(x => (x,newMap(x)))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val newMap = y.toMap.withDefaultValue(0)
    x.map( a => (a._1,a._2-newMap(a._1))).filter(a => a._2>0)
  }
